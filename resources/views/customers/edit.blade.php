    ...
    <div class="space-y-6">
        ...
        <nav class="text-sm font-medium text-indigo-900/60">
            ...
            <a href="{{ route('customers.show', $customer) }}"
                class="hover:text-indigo-600 transition-colors">{{ $customer->name }}</a>
            <span class="mx-2 text-indigo-300">/</span>
        ...
        <div class="flex items-center space-x-4">
            ...
            <div>
                <h1 class="text-2xl font-bold text-indigo-950 tracking-tight">Edit Customer</h1>
                <p class="text-sm text-indigo-500 font-medium">{{ $customer->name }}</p>
            </div>
        </div>

        <!-- Form -->
        <x-card class="border border-indigo-100 shadow-lg ring-1 ring-indigo-50/50">
            <form method="POST" action="{{ route('customers.update', $customer) }}" class="space-y-8">
                @csrf
                @method('PUT')
