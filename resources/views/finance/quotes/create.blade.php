...
@section('content')
<div class="max-w-4xl mx-auto py-8 px-4 sm:px-6 lg:px-8" x-data="quoteForm()">
    <x-flash-message />

    <h1 class="text-2xl font-semibold text-gray-900 mb-6">New quote</h1>
    ...
    <form method="post" action="{{ route('quotes.store') }}" class="space-y-6">
        ...
        <div class="bg-white shadow rounded-lg p-6 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                    ...
                    <select name="customer_id" required class="w-full rounded border-gray-300 text-sm">
                ...
                <div>
                    ...
                    <input type="date" name="issue_date" value="{{ old('issue_date', now()->toDateString()) }}"
                           required class="w-full rounded border-gray-300 text-sm">
                </div>
                <div>
            ...
            <div>
                ...
                <textarea name="notes" rows="2" class="w-full rounded border-gray-300 text-sm">{{ old('notes') }}</textarea>
            </div>
        </div>
                    ...
                    <template x-for="(item, idx) in items" :key="idx">
                        <tr class="border-t border-gray-100">
                            <td><input type="text" :name="`items[${idx}][description]`" x-model="item.description"
                                       required class="w-full rounded border-gray-300 text-sm"></td>
                            <td><input type="number" min="1" :name="`items[${idx}][quantity]`" x-model.number="item.quantity"
                                       required class="w-full rounded border-gray-300 text-sm text-right"></td>
        ...
        <div class="flex justify-end gap-2">
            ...
            <button type="submit" class="px-4 py-2 rounded bg-accent-500 text-white text-sm hover:bg-accent-600">Create quote</button>
        </div>
    </form>
</div>
