                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-indigo-50 p-6">
                        <h3 class="text-xs font-bold text-indigo-400 uppercase tracking-wider mb-4">Customer & Vehicle</h3>
                         <div class="flex items-start gap-4 mb-4">
                            ...
                            <div>
                                <h4 class="font-bold text-indigo-900">{{ $workOrder->customer->name ?? 'Unknown Customer' }}</h4>
                                <p class="text-sm text-indigo-500">{{ $workOrder->customer->phone ?? 'No Phone' }}</p>
                            </div>
                        </div>
                        <div class="border-t border-indigo-50 pt-4">
                            <p class="text-sm text-indigo-900 font-medium">{{ $workOrder->vehicle->make ?? 'Unknown' }} {{ $workOrder->vehicle->model ?? '' }}</p>
                            <p class="text-xs text-indigo-400 mt-1">Plate: {{ $workOrder->vehicle->license_plate ?? 'N/A' }}</p>
                        </div>
                    </div>
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-indigo-50 p-6">
                        <div class="flex items-center justify-between mb-4">
                            ...
                            <button type="submit" class="px-4 py-2 bg-indigo-600 text-white rounded-lg font-bold text-sm hover:bg-indigo-700 transition-colors shadow-sm">
                                Save Notes
                            </button>
                        </div>
                    ...
                    <div class="bg-white rounded-xl shadow-sm border border-indigo-50 p-6">
                        <div class="flex items-center justify-between">
                            ...
                            <a href="{{ route('work-orders.index') }}" 
                               class="px-6 py-3 border border-indigo-200 text-indigo-600 rounded-lg font-bold hover:bg-indigo-50 transition-colors">
                                Back to Work Orders
                            </a>
                        </div>
                    </div>
